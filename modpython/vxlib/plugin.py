...
function! s:Exception(throwpoint, exception, plugid, loadstatus)
   call vxlib#plugin#Exception(s:thisfile, a:throwpoint, a:exception, a:plugid, a:loadstatus)
endfunc
...
function! s:Exception(throwpoint, exception, plugid, loadstatus)
   ...
   call add(g:vxlib_exception_list, matchstr(a:throwpoint, ',\s*\zsline\s\+\d\+') . ' (' . a:plugid . '):')
...
'''
file_tail ='''\
"delfunc s:CheckSetting
"delfunc s:IsEnabled
"delfunc s:GetLoadStatus
"delfunc s:SetLoaded
"delfunc s:SetEnabledDefault
"delfunc s:ContinueLoading
"delfunc s:PluginExists
"unlet s:thisfile
'''
...
def parseFile(fn):
    ...
    while True:
        ...
        if mo != None:
            ...
            if ln == None: break
            continue
        # PLUGINFUNCTION deprecated; use autoload functions, instead.
...
def openFileToGenerate(outfile):
    try: out = open(outfile, "w")
    except IOError: out = None

    if out == None:
        p1 = os.path.dirname(outfile)
...
def writePluginCode(plugins, functions):
    ...
    if options.outfile == "": out = CStdout()
        ...
        out = openFileToGenerate(options.outfile)
        if out == None:
            return
