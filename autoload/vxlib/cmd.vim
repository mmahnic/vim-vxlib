...
if vxlib#load#IsLoaded( '#vxlib#cmd' )
   finish
endif
call vxlib#load#SetLoaded( '#vxlib#cmd', 1 )
...
function! vxlib#cmd#TransformQFixItems(items)
   let lastbuf = -1 
   let vxitems = []
   let vxids = []
   let i = 0
   for err in a:items
      ...
      call add(vxids, i)
      let type = substitute(err.type, '\W', '', 'g') " helpgrep returns ^A
      call add(vxitems, printf(' %2d: %3d %s %s', i, err.lnum, type, err.text))
   endfor
